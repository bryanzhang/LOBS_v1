...
def optimal_brain_surgeon_v1(layer, indices, h_block):
    flat_weight = layer.weight.flatten()
    ...
    beta = original_beta[row_mask]
    assert beta.size(0) == layer.in_features * layer.out_features - indices.size(0)
    # 求裁剪之后的各个子逆阵
    ...
    for output_no in range(0, layer.out_features):
        ...
        inv = torch.linalg.pinv(subblock)
        hpinvs.append(inv)
    beta = beta.contiguous()
    delta_star = custom_kernels.hessianorpinv_matmul_vector_v1(hpinvs, beta)
